...
import { useRegister, useResendVerification, useGoogleOAuthUrl } from '../../../../hooks/useAuth'
  ...
  const handleGoogleSignup = async () => {
    try {
      ...
      if (data?.success && 'data' in data) {
        ...
        setApiError('Failed to initiate Google sign up. Please try again.')
      }
    } catch (error) {
  ...
  return (
    <div className={`w-full max-w-md mx-auto p-6 backdrop-blur-xl bg-white/80 dark:bg-dark-surface/80 rounded-2xl shadow-2xl border border-gray-200/50 dark:border-gray-700/50 ${isMounted ? 'animate-fade-in-left' : 'opacity-0'}`}>
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-4">
        ...
        >
          {isGettingGoogleUrl ? 'Loading...' : 'Sign up with Google'}
        </Button>
